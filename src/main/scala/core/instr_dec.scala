...
case class instr_dec(param: CPU_PARAM) extends Component {
    ...
    io.alu_op_add  := logic_add | logic_add_imm | op_auipc | op_lui |op_load | op_store
    ...
    // == Other control signal == //
    io.op1_sel_pc           := op_auipc
    io.op2_sel_imm          := (op_logic_arithm_imm | op_load | op_store | op_lui | op_auipc)
    io.op1_sel_zero         := op_lui
    io.branch_op            := op_branch
    io.register_wr          := (op_logic_arithm | op_logic_arithm_imm | op_load | op_lui | op_auipc)
    io.register_rs1_rd      := (op_logic_arithm | op_logic_arithm_imm | op_load | op_store)
    io.register_rs2_rd      := op_logic_arithm | op_store
