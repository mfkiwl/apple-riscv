...
case class clic(soc_param: SOC_PARAM) extends Component {
  ...
  val mtime       = busCtrl.createWriteAndReadMultiWord(UInt(soc_param.CLIC_TIMER_WIDTH bits), 4,
      "Free running timer, when value reaches mtimecmp, timer interrupt will be fired") init 0
  val mtimecmp    = busCtrl.createWriteAndReadMultiWord(UInt(soc_param.CLIC_TIMER_WIDTH bits), 0xC,
